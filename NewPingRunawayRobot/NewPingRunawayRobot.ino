/* NewPingRunawayRobot.ino - Created by Rashaad Thomas
  ...
  * Parallax Ping))) Ultrasonic Sensor connected to TKD3 pin, displayed as D3 on Control 
  ...
  Please note: An Arduino Uno (Rev 3) was used as an ISP, or In-System Programmer (see ArduinoISP program example), to bypass 
...
#define MAX_DISTANCE 2000 // Maximum distance we want to ping for (in centimetres).
...
void loop() {
  ...
  while (getDistance() < 40) { // If an obstacle is less than 40cm away
